...
def get_zone_name(domain):
    ...
    for z in zones:
        if domain.endswith(z.name) and z.name.count(".") > zone_name.count(
            "."
        ):
            zone_name = z.name
    if not zone_name:
...
def create_txt_record(domain, token, account_number):
    ...
    try:
        ...
        if "Cannot duplicate existing record data" in e.message:
            current_app.logger.debug(
                f"Unable to add record. Domain: {domain}. Token: {token}. Record already exists: {e}",
                exc_info=True,
            )
...
def delete_txt_record(change_id, account_number, domain, token):
    ...
    try:
        ...
        metrics.send(
            ...
            "counter",
            1,
            metric_tags={"fqdn": fqdn, "txt_record": str(txt_record.txtdata)},
        )
...
def delete_acme_txt_records(domain):
    ...
    if not domain.startswith(acme_challenge_string):
        current_app.logger.debug(
            f"delete_acme_txt_records: Domain {domain} doesn't start with string {acme_challenge_string}. Cowardly refusing to delete TXT records"
        )
...
def get_authoritative_nameserver(domain):
    if not current_app.config.get("ACME_DYN_GET_AUTHORATATIVE_NAMESERVER"):
        return "8.8.8.8"
